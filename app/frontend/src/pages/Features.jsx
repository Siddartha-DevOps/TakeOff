    ...
    <>
      ...
      <section className="relative overflow-hidden gradient-soft-bg py-24">
        ...
        >
          ...
          <p className="mt-5 text-lg text-slate-600 max-w-2xl mx-auto">
            From automated takeoffs to real-time collaboration and semantic plan search,
            TakeOff replaces half a dozen tools with one.
          </p>
        </motion.div>
      </section>
      ...
      <section className="max-w-7xl mx-auto px-6 lg:px-8 py-24 space-y-32">
        {FEATURES.map((f, idx) => {
          ...
          const a = ACCENTS[f.accent] || ACCENTS.indigo;
          ...
          return (
            ...
            >
              ...
              <div>
                <div
                  className={`w-11 h-11 rounded-xl flex items-center justify-center ring-8 ${a.bg} ${a.text} ${a.ring}`}
                >
                  <Icon className="w-5 h-5" />
                </div>

                <div className="mt-5 text-[11px] uppercase tracking-wider text-slate-500 font-semibold">
                  {f.kicker}
                </div>
                ...
                <Link
                  to="/demo"
                  className="mt-7 inline-flex items-center gap-1.5 text-sm font-medium text-slate-900 hover:text-indigo-600 transition-colors"
                >
              ...
              <motion.div
                ...
                className="relative rounded-2xl bg-white border border-slate-200 shadow-xl overflow-hidden aspect-[4/3]"
              >
                <FloorPlanCanvas autoplay={idx === 0} />
                <div
                  className={`absolute top-3 left-3 px-2.5 py-1 rounded-md text-[11px] font-medium ${a.bg} ${a.text}`}
                >
                  {f.kicker}
                </div>
              </motion.div>
