...
import { Check, Sparkles, ChevronDown, ArrowRight, Loader2 } from 'lucide-react';
...
export default function Pricing() {
  ...
  const handleSubscribe = async (planName) => {
    ...
    // Business plan redirects to contact
    if (planName === 'Business') {
      window.location.href = 'mailto:sales@takeoff.ai';
      return;
    }

    // Map plan name to package_id
    const packageId = planName.toLowerCase(); // "starter" or "growth"

    try {
      setLoading(planName);
      
      // Get origin URL from browser
      const originUrl = window.location.origin;
      
      // Create checkout session
      const response = await paymentsAPI.createCheckoutSession(packageId, originUrl);
          ...
          {PRICING_PLANS.map((p) => (
            <div key={p.name} className={`relative rounded-2xl p-8 ${p.highlight ? 'bg-slate-900 text-white shadow-2xl shadow-slate-900/30 ring-1 ring-slate-900' : 'bg-white border border-slate-200'}`}>
              ...
              <button
                ...
                className={`mt-6 w-full flex items-center justify-center gap-2 py-2.5 rounded-lg text-sm font-medium ${
                  p.highlight 
